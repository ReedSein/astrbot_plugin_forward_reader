...
*   **本质的嗅探**: 在这单調的表象之下，这只标本散发出的，是何种气味？是[试探]、[天真]、[无聊]、[亵渎]，还是[对我存在的挑衅]？
...
"""
    ...
    # ==================== [新增功能] ====================
    # 新增一个事件处理器，用于在LLM响应后进行清理
    @filter.is_self(priority=99)
    async def cleanup_rosa_response(self, event: AstrMessageEvent):
        """
        拦截机器人自己的消息，如果包含罗莎的内心OS，则清理掉只保留最终回复。
